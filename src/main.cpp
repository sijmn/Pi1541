...
extern "C"
{
	void kernel_main(unsigned int r0, unsigned int r1, unsigned int atags)
	{
		...
		char ipBuffer[USPI_FRAME_BUFFER_SIZE];
		if (USPiEthernetAvailable()) {
			...
			Ipv4Packet<decltype(icmp)> ip(1, ipAddress, 0x0A00000A, icmp);

			EthernetFrame<decltype(ip)> frame(0x0800, ip);
			USPiGetMACAddress(frame.macSource);
